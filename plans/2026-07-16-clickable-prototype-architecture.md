...
### [x] Фаза 5 — Сквозная отзывчивость + мягкие тени
Все интерактивные элементы получают hover/focus/active; карточки/попапы — глубину.
- globals: расширены селекторы переходов (`[class*="ds-"]`, роли checkbox/radio/
  option/link, `[tabindex="0"]`, `[data-clickable]`); добавлены переменные теней
  `--shadow-xs/sm/md/lg/hover` (subtle, светлая тема).
- Тени глубины: карточки (`.ds-account/.ds-achievement/.ds-domain/.ds-qcard` +
  Tailwind-карточки member/launcher/basis/ckp/feed-post/profile/option/info/
  verification) — `shadow-sm`, кликабельные — `hover:shadow-hover`. Попапы:
  toast/dropdown-menu `shadow-lg`, tooltip `shadow-md`.
- Hover где не было: строки doc-row (history-operations, validator-processed),
  футер-ячейки сайдбаров (MenuFooter «Admin»/«Пред. правления»), иконки-кнопки
  ckp-card (ред./вид), chat back/close.
- Проверено, что уже было (не дублировали): textarea focus (общие `.ds-field`),
  calendar/incriment/combobox hover, left-menu nav, tabs, header icon-buttons,
  select-option. Декоративные (flag/badge) — намеренно без hover.
Проверка: `tsc` + `next build` — оба exit 0.

### [ ] Фаза 6 — Reveal / карусель (по требованию)
